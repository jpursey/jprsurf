...
reaper_csurf_reg_t* PluginSurface::GetControlSurfaceReg() {
  static reaper_csurf_reg_t reg = {kTypeString, kDescString,
                                   PluginSurface::Create,
                                   PluginSurface::ShowConfig};
  return &reg;
}

IReaperControlSurface* PluginSurface::Create(const char* type_string,
  ...
  VLOG_REAPER() << "Create(type_string="
                ...
                << "\")";
  return new PluginSurface(type_string != nullptr ? type_string : "",
                           config_string != nullptr ? config_string : "");
}

HWND PluginSurface::ShowConfig(const char* type_string, HWND parent,
...
PluginSurface::PluginSurface(std::string type_string, std::string config_string)
...
const char* PluginSurface::GetTypeString() { return "JPRSurf"; }

const char* PluginSurface::GetDescString() { return kDescString; }

const char* PluginSurface::GetConfigString() {
...
void PluginSurface::SetTrackListChange() {
  VLOG_REAPER() << "SetTrackListChange";
  track_list_changed_ = true;
}

void PluginSurface::SetSurfaceVolume(MediaTrack* track_id, double volume) {
  VLOG_REAPER() << "SetSurfaceVolume(track_id=" << track_id
                << ", volume=" << volume << ")";
}

void PluginSurface::SetSurfacePan(MediaTrack* track_id, double pan) {
  VLOG_REAPER() << "SetSurfacePan(track_id=" << track_id << ", pan=" << pan
                << ")";
}

void PluginSurface::SetSurfaceMute(MediaTrack* track_id, bool mute) {
  VLOG_REAPER() << "SetSurfaceMute(track_id=" << track_id << ", mute=" << mute
                << ")";
}

void PluginSurface::SetSurfaceSelected(MediaTrack* track_id, bool selected) {
...
void PluginSurface::SetSurfaceSolo(MediaTrack* track_id, bool solo) {
  VLOG_REAPER() << "SetSurfaceSolo(track_id=" << track_id << ", solo=" << solo
                << ")";
}

void PluginSurface::SetSurfaceRecArm(MediaTrack* track_id, bool rec_arm) {
  VLOG_REAPER() << "SetSurfaceRecArm(track_id=" << track_id
                << ", rec_arm=" << rec_arm << ")";
}

void PluginSurface::SetPlayState(bool play, bool pause, bool rec) {
  VLOG_REAPER() << "SetPlayState(play=" << play << ", pause=" << pause
                << ", rec=" << rec << ")";
}

void PluginSurface::SetRepeatState(bool rep) {
  VLOG_REAPER() << "SetRepeatState(rep=" << rep << ")";
}

void PluginSurface::SetTrackTitle(MediaTrack* track_id, const char* title) {
  VLOG_REAPER() << "SetTrackTitle(track_id=" << track_id << ", title=\""
                << (title != nullptr ? title : "(null)") << "\")";
}

bool PluginSurface::GetTouchState(MediaTrack* track_id, int is_pan) {
  VLOG_REAPER() << "GetTouchState(track_id=" << track_id
                << ", is_pan=" << is_pan << ")";
  return false;
}

void PluginSurface::SetAutoMode(int mode) {
  VLOG_REAPER() << "SetAutoMode(mode=" << mode << ")";
  TrackCache::Get().OnAutoModeChanged();
}

void PluginSurface::ResetCachedVolPanStates() {
  VLOG_REAPER() << "ResetCachedVolPanStates";
}

void PluginSurface::OnTrackSelection(MediaTrack* track_id) {
  VLOG_REAPER() << "OnTrackSelection(track_id=" << track_id << ")";

  // TODO: Ensure track is in view
}

bool PluginSurface::IsKeyDown(int key) {
...
int PluginSurface::Extended(int call, void* param1, void* param2,
                            void* param3) {
  switch (call) {
...
void PluginSurface::OnReset() { VLOG_REAPER() << "OnReset"; }

void PluginSurface::OnSetInputMonitor(MediaTrack* track_id, int rec_monitor) {
  VLOG_REAPER() << "OnSetInputMonitor(track_id=" << track_id
                << ", rec_monitor=" << rec_monitor << ")";
}

void PluginSurface::OnSetMetronome(bool enabled) {
  VLOG_REAPER() << "OnSetMetronome(enabled=" << enabled << ")";
}

void PluginSurface::OnSetAutoRecArm(bool auto_rec_arm) {
  VLOG_REAPER() << "OnSetAutoRecArm(auto_rec_arm=" << auto_rec_arm << ")";
}

void PluginSurface::OnSetRecMode(int rec_mode) {
  VLOG_REAPER() << "OnSetRecMode(rec_mode=" << rec_mode << ")";
}

void PluginSurface::OnSetSendVolume(MediaTrack* track_id, int send_idx,
                                    double volume) {
  VLOG_REAPER() << "OnSetSendVolume(track_id=" << track_id
                << ", send_idx=" << send_idx << ", volume=" << volume << ")";
}

void PluginSurface::OnSetSendPan(MediaTrack* track_id, int send_idx,
                                 double pan) {
  VLOG_REAPER() << "OnSetSendPan(track_id=" << track_id
                << ", send_idx=" << send_idx << ", pan=" << pan << ")";
}

void PluginSurface::OnSetFxEnabled(MediaTrack* track_id, int fx_idx,
                                   bool enabled) {
  VLOG_REAPER() << "OnSetFxEnabled(track_id=" << track_id
                << ", fx_idx=" << fx_idx << ", enabled=" << enabled << ")";
}

void PluginSurface::OnSetFxParam(MediaTrack* track_id, int fx_idx,
...
void PluginSurface::OnSetFxParamRecFx(MediaTrack* track_id, int fx_idx,
                                      int param_idx, double normalized_value) {
...
void PluginSurface::OnSetBpmAndPlayRate(std::optional<double> bpm,
...
void PluginSurface::OnClearLastTouchedFx() {
  VLOG_REAPER() << "OnClearLastTouchedFx";
}

void PluginSurface::OnSetLastTouchedFx(MediaTrack* track_id,
...
void PluginSurface::OnClearFocusedFx() { VLOG_REAPER() << "OnClearFocusedFx"; }

void PluginSurface::OnSetFocusedFx(MediaTrack* track_id,
                                   std::optional<int> media_item_idx,
                                   int fx_idx) {
...
void PluginSurface::OnSetLastTouchedTrack(MediaTrack* track_id) {
...
void PluginSurface::OnSetMixerScroll(MediaTrack* track_id) {
  VLOG_REAPER() << "OnSetMixerScroll(track_id=" << track_id << ")";
}

void PluginSurface::OnSetPanEx(MediaTrack* track_id,
                               absl::Span<const double> pan, int mode) {
  VLOG_REAPER() << "OnSetPanEx(track_id=" << track_id << ", pan=["
                << absl::StrJoin(pan, ", ") << "], mode=" << mode << ")";
}

void PluginSurface::OnSetRecvVolume(MediaTrack* track_id, int rec_idx,
                                    double volume) {
  VLOG_REAPER() << "OnSetRecvVolume(track_id=" << track_id
                << ", rec_idx=" << rec_idx << ", volume=" << volume << ")";
}

void PluginSurface::OnSetRecvPan(MediaTrack* track_id, int rec_idx,
                                 double pan) {
  VLOG_REAPER() << "OnSetRecvPan(track_id=" << track_id
                << ", rec_idx=" << rec_idx << ", pan=" << pan << ")";
}

void PluginSurface::OnSetFxOpen(MediaTrack* track_id, int fx_idx, bool open) {
  VLOG_REAPER() << "OnSetFxOpen(track_id=" << track_id << ", fx_idx=" << fx_idx
                << ", open=" << open << ")";
}

void PluginSurface::OnSetFxChange(MediaTrack* track_id, int flags) {
  VLOG_REAPER() << "OnSetFxChange(track_id=" << track_id
                << ", flags=" << absl::Hex(flags, absl::kZeroPad8) << ")";
}

void PluginSurface::OnSetProjectMarkerChange() {
  VLOG_REAPER() << "OnSetProjectMarkerChange";
}

void PluginSurface::OnTrackFxPresetChanged(MediaTrack* track_id, int fx_idx) {
  VLOG_REAPER() << "OnTrackFxPresetChanged(track_id=" << track_id
                << ", fx_idx=" << fx_idx << ")";
}

bool PluginSurface::OnSupportsExtendedTouch() {
  VLOG_REAPER() << "OnSupportsExtendedTouch() -> false";
  return false;
}

void PluginSurface::OnMidiDeviceRemap(bool is_out, int old_idx, int new_idx) {
